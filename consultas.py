...
Menu.OP1.value.comando = f'''SELECT IdLoja, IdCarro, AnoFab, ValorDiaria, TipoCombustivel, Placa, Opcionais
    ...
    WHERE  IdCarro NOT IN (SELECT IdCarro FROM Locacoes WHERE DataFinal IS NULL) $USER_SELECT
    GROUP BY IdLoja,NomeCat,IdCarro,AnoFab, ValorDiaria, TipoCombustivel, Placa, Opcionais
    ORDER BY IdLoja;
'''

Menu.OP2.value.select = ""
...
Menu.OP7.value.comando = f'''SELECT IdLocatario, Nome, Email, Telefone, CARROS1.IdCarro, NomeCat, Placa, ValorDiaria, DataInicio, DataDevPrevista
    ...
    WHERE DataFinal IS NULL $USER_SELECT;
'''
...
Menu.OP8.value.comando = f'''SELECT DISTINCT Nome, Email, Telefone
        ...
        WHERE GERENTES.IdFuncionario IS NOT NULL OR MECANICOS.IdFuncionario IS NOT NULL $USER_SELECT
'''

Menu.OP9.value.select = ""
...
def formatar_consulta(opcao, args = ""):
    ...
    comando = Menu.get_opcao(opcao).value.comando
    return comando.replace("$USER_SELECT", f"AND {args}")
...
def main():
    ...
    if(len(Menu.get_opcao(opcao).value.select) > 0):
    	print(f"{Menu.get_opcao(opcao).value.select} \n\nDentre os valores acima, especifique os que deseja restringir. Para multiplos valores, separe-os por vírgula (','). Ou enter se não deseja especificar a consulta")
    	selecao = input()

    comando = formatar_consulta(opcao, selecao)


    conexao.execute(comando)  # realiza a query
